...
const createCertificate = (studentName: string, courseName: string, date: string, fileName: string) => {
    ...
    // Content
    doc.font(`${fontDir}/SUSE-Regular.ttf`).fontSize(16).fill('#021c27').text('CERTIFICATE OF COMPLETION', {
        align: 'center',
    });

    jumpLine(doc, 1);

    doc.font(`${fontDir}/SUSE-Light.ttf`).fontSize(10).fill('#021c27').text('Present to', {
        align: 'center',
    });

    jumpLine(doc, 2);

    doc.font(`${fontDir}/SUSE-Bold.ttf`).fontSize(24).fill('#021c27').text(studentName, {
        align: 'center',
    });

    jumpLine(doc, 1);

    doc.font(`${fontDir}/SUSE-Light.ttf`)
        .fontSize(10)
        .fill('#021c27')
    ...
    doc.font(`${fontDir}/SUSE-Bold.otf`)
        .fontSize(10)
        .fill('#021c27')
    ...
    doc.font(`${fontDir}/SUSE-Light.otf`)
        .fontSize(10)
        .fill('#021c27')
